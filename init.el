...
(defun backspace-or-unindent() (interactive)
	(cond
        (mark-active (call-interactively 'kill-region))
...
(define-key vi-mode-map (kbd "C-a") 'forward-to-word)
(define-key vi-mode-map (kbd "C-i") 'backward-word)
(define-key vi-mode-map (kbd "r") 'zap-to-char)
(define-key vi-mode-map (kbd "t") 'delete-forward-char)
(define-key vi-mode-map (kbd "T") 'kill-word)
(define-key vi-mode-map (kbd "n") 'backspace-or-unindent)
(define-key vi-mode-map (kbd "N") 'backward-kill-word)
(define-key vi-mode-map (kbd "C-n") 'kill-line)
(define-key vi-mode-map (kbd ".") 'repeat)
(define-key vi-mode-map (kbd "j") 'join-line)
(define-key vi-mode-map (kbd "s") 'set-mark-command)
